#!/system/bin/sh
# Each phase keeps LITTLE core0 as system core baseline and shields prime cores.
#   (0+8) idle → stress
...
if [ -n "$MINMAX" ]; then
  LOG_PREFIX="$OUT_DIR/prime_core_power_${MINMAX}"
else
  LOG_PREFIX="$OUT_DIR/prime_core_power"
fi
...
PRIME_CORES="8"
SYSTEM_CORE="0"             # baseline LITTLE core for system tasks
...
setup_cgroups() {
  ...
  # Initially, system gets SYSTEM_CORE only
...
run_idle_phase() {
  PHASE_NUM="$1"
  CORE_CONFIG="$2"  # Space-separated list of cores "0 8"
  
  # Convert core list to CPU_SET string (e.g., "0+6")
  ...
  # Shield only the non-system cores (prime cores in this case)
  # Core 0 remains as system core and is NOT shielded
  CORES_TO_SHIELD=""
  for c in $CORE_CONFIG; do
  ...
  CORES_TO_SHIELD="$(echo "$CORES_TO_SHIELD" | sed 's/^ *//')"  # trim leading space
...
run_stress_phase() {
  ...
  for c in $CORE_CONFIG; do
    ...
    case "$c" in
        8) mask="100" ;;     # core 8 → 0x100
        *) mask=$((1<<c)) ;; # fallback if you ever reuse this
    esac


    pid="$(start_yes_mask "$mask")"
    echo "PID=$pid on CPU$c (mask=$mask)"
   done
...
# ---- Run sequence -----------------------------------------------------------
echo "=== Prime Cluster Incremental Power Test (SHIELDED) ==="
echo "Prime cores: $PRIME_CORES"
...
# CPU 0+8
run_idle_phase $PHASE "0 8"
PHASE=$((PHASE + 1))

run_stress_phase $PHASE "0 8"
PHASE=$((PHASE + 1))
