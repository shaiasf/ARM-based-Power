#!/system/bin/sh
# prime_cluster_power.sh – Three-phase power test for PRIME cluster:
# Phase 0: Only CPU0 online, idle (baseline)
# Phase 1: All PRIME cores online, idle (no stress)
# Phase 2: All PRIME cores online, stressed
# Root required.
# Usage:
#   adb push prime_cluster_power.sh /data/local/tmp/
#   adb shell
#   su
#   sh /data/local/tmp/prime_cluster_power.sh [duration_sec=600] [interval_s=0.5] [idle_gap_sec=300]
...
PRIME_CORES="8"                 # Define PRIME cluster cores
...
run_idle_phase() {
  echo "[+] Phase 1 (idle): PRIME core (8) online idle, CPU0 housekeeping online idle"

  # Bring all PRIME cores online
  for c in $PRIME_CORES; do
    set_cpu_online "$c" 1
  done
  ...
  # Turn off all other cores (not in PRIME_CORES or HOUSEKEEPING_CORES)
  for c in $ALL_CORES; do
    if ! echo "$PRIME_CORES $HOUSEKEEPING_CORES" | grep -qw "$c"; then
      set_cpu_online "$c" 0
    fi
  done
...
run_stress_phase() {
  echo "[+] Phase 2 (stress): PRIME core (8) online and stressed, CPU0 housekeeping online idle"

  # Ensure all PRIME cores are online
  for c in $PRIME_CORES; do
    set_cpu_online "$c" 1
  done
  ...
  # Start stress ONLY on PRIME cores (8), NOT on housekeeping core (0)
  PIDS=""
  for c in $PRIME_CORES; do
    case "$c" in
      6) mask="40" ;;
