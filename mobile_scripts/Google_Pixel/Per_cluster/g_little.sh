#!/system/bin/sh
# little_cluster_power.sh – Two-phase power test for LITTLE cluster with CPU shielding:
# Phase 0: All LITTLE cores online, idle (no stress)
# Phase 1: All LITTLE cores online, stressed
# Uses cgroup cpusets to isolate cores from system tasks
# Root required.
# Usage:
#   adb push little_cluster_power.sh /data/local/tmp/
#   adb shell
#   su
#   sh /data/local/tmp/little_cluster_power.sh [duration_sec=600] [interval_s=0.5] [idle_gap_sec=300]
...
setup_cpusets() {
  ...
  echo "0-8" > "$CGROUP_SYSTEM/cpus" 2>/dev/null || true
  echo "0" > "$CGROUP_SYSTEM/mems" 2>/dev/null || true
  echo "0-8" > "$CGROUP_SHIELD/cpus" 2>/dev/null || true
...
update_shield_cores() {
  ...
  # Convert space-separated list to range format (e.g., "0 1 2 3" -> "0-3")
...
run_idle_phase() {
  echo "[+] Phase 0 (idle): All LITTLE cores online, no stress"

  # Setup cpusets for shielding
  setup_cpusets
  move_tasks_to_system

  # Bring all LITTLE cores online
  for c in $LITTLE_CORES; do
    set_cpu_online "$c" 1
  done
  # Turn off big cores (assuming cores 4-8 are big/prime)
  for c in $ALL_CORES; do
    if ! echo "$LITTLE_CORES" | grep -qw "$c"; then
      set_cpu_online "$c" 0
    fi
  done
  
  # Update shield to include LITTLE cores
  update_shield_cores "$LITTLE_CORES"

  LOG="${LOG_PREFIX}.csv"
  CPU_SET="$(echo "$LITTLE_CORES" | tr ' ' '+')"
  RES="$(sample_power_csv phase0_idle "$CPU_SET" "$LOG")"
  AVG_PWR="$(echo "$RES" | awk '{print $1}')"
  AVG_BATT_TEMP="$(echo "$RES" | awk '{print $2}')"
  AVG_CPU_TEMP="$(echo "$RES" | awk '{print $3}')"
  AVG_FREQS="$(echo "$RES" | cut -d' ' -f4-)"

  echo "[RESULT] Phase 0 (idle): avg_power_W=$AVG_PWR | avg_batt_temp_C=$AVG_BATT_TEMP | avg_cpu_temp_C=$AVG_CPU_TEMP | $AVG_FREQS"
}

run_stress_phase() {
  echo "[+] Phase 1 (stress): All LITTLE cores online and stressed"
  # Ensure all LITTLE cores are online
  for c in $LITTLE_CORES; do
    set_cpu_online "$c" 1
  done
  
  # Start stress on all LITTLE cores and move to shield cgroup
  PIDS=""
  for c in $LITTLE_CORES; do
    mask=$((1 << c))
  ...
  LOG="${LOG_PREFIX}.csv"
  CPU_SET="$(echo "$LITTLE_CORES" | tr ' ' '+')"
  RES="$(sample_power_csv phase1_stress "$CPU_SET" "$LOG")"
  ...
  echo "[RESULT] Phase 1 (stress): avg_power_W=$AVG_PWR | avg_batt_temp_C=$AVG_BATT_TEMP | avg_cpu_temp_C=$AVG_CPU_TEMP | $AVG_FREQS"
}

# ---- Run sequence ----------------------------------------------------------
echo "=== LITTLE Cluster Power Test ==="
