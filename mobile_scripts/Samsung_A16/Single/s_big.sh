#!/system/bin/sh
# Each phase keeps LITTLE core0 as system core baseline and shields big cores.
#   (0+6) idle → 6 stressed
#   (0+7) idle → 7 stressed
# Root required.
...
run_idle_phase() {
  PHASE_NUM="$1"
  CORE_CONFIG="$2"  # Space-separated list of cores, e.g., "0 6" or "0 7"
  ...
  # Shield only the non-system cores (big cores in this case)
...
run_stress_phase() {
  ...
  for c in $CORE_CONFIG; do
    ...
    mask=$((1<<c)); [ "$c" = "6" ] && mask="40"; [ "$c" = "7" ] && mask="80"
    pid="$(start_yes_mask "$mask")"; echo "PID=$pid on CPU$c"
  done
...
# ---- Run sequence -----------------------------------------------------------
echo "=== BIG Cluster Incremental Power Test (SHIELDED) ==="
...
# CPU 0+6
run_idle_phase $PHASE "0 6"
PHASE=$((PHASE + 1))

run_stress_phase $PHASE "0 6"
PHASE=$((PHASE + 1))
idle_gap_wait

# CPU 0+7
run_idle_phase $PHASE "0 7"
PHASE=$((PHASE + 1))

run_stress_phase $PHASE "0 7"
PHASE=$((PHASE + 1))
