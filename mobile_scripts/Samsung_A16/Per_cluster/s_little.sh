...
if [ -n "$MINMAX" ]; then
  LOG_PREFIX="$OUT_DIR/little_core_power_cluster_${MINMAX}"
else
  LOG_PREFIX="$OUT_DIR/little_core_power_cluster"
fi
...
update_shield_cores() {
  ...
  # Convert space-separated list to comma format (e.g., "0 1 2 3 4 5" -> "0-5")
...
run_stress_phase() {
  ...
  # Start stress on all LITTLE cores with explicit hex masks and move to shield cgroup
  PIDS=""
  for c in $LITTLE_CORES; do
    case "$c" in
      0) mask="1" ;;
...
echo "=== LITTLE Cluster Power Test (Samsung A16) ==="
